...
function print_backtrace_header( $backtrace ) {
	...
	if ( isset($backtrace[1]) && $backtrace[1]['function'] !== 'global_exception_handler' && isset($backtrace[1]) && isset($backtrace[1]['class']) ) {
	...
	} else if ( isset($backtrace[1]) && $backtrace[1]['function'] !== 'global_exception_handler' ) {
...
function __constructStatic($name) {
	...
	if ( method_exists($name, '__static') ) {
		...
		// since our static constructors do not share the same concept of inheritence
		...
		// children of
		if (stripos($reflection->getDocComment(), '@polymorphic') !== false ||
...
function __($mixed) {
	if (is_integer($mixed)) {
		...
		return new \eGloo\Utilities\ActiveSupport\Collection($mixed);
