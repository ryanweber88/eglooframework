		...
		if ($this->_queryExecutionSteps[$queryName]['parameterOrderMatters']) {
			...
			foreach( $parametersNeeded as $parameter ) {
				if ( $parameter['source'] === 'GET' ) {
					$newParameter = array('type' => $parameter['type'], 'value' => $this->bean->getGET($parameter['value']));
				} else if ( $parameter['source'] === 'POST' ) {
					$newParameter = array('type' => $parameter['type'], 'value' => $this->bean->getPOST($parameter['value']));
