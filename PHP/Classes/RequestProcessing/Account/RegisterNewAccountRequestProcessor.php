...
class RegisterNewAccountRequestProcessor extends RequestProcessor {
    ...
    public function processRequest() {
        ...
        $requestedPassword = $this->bean->getPOST('userRequestedAccountPassword');
        $confirmedPassword = $this->bean->getPOST('userConfirmedAccountPassword');

		$daoFactory = AbstractDAOFactory::getInstance();
		$userInvitesDAO = $daoFactory->getUserInvitesDAO();


        if ( $this->bean->getRequestID() !== 'registerNewAccount' ) {
            // Throw exception
            // TODO throw exception; this is a programmer error
        }

        //check if they accept the user license
        else if ( !$this->bean->issetPOST('userAcceptsLicense') ) {
        ...
        else if( ! $userInvitesDAO->isReferralCodeValid( $this->bean->getPOST('userReferralCode') ) ){
            $accountDTO = new AccountDTO();
            $accountDTO->setRegistrationSuccessful( false );
    ...
    private function processLogin(){
        ...
        $accountName = $this->bean->getPOST('userPreferredAccountName');
        $password = $requestedPassword = hash(self::STRONG_HASH, $this->bean->getPOST('userRequestedAccountPassword') );

        $userEmail = $this->bean->getPOST('userEmail');
        $firstName = $this->bean->getPOST('firstNameInput');
        $lastName = $this->bean->getPOST('lastNameInput');
        $gender = $this->bean->getPOST('userGender');

        $birthMonth = $this->bean->getPOST('userBirthMonth');
        $birthDay = $this->bean->getPOST('userBirthDay');
        $birthYear = $this->bean->getPOST('userBirthYear');
        $referralCode = $this->bean->getPOST('userReferralCode');
		...
		if( $accountDTO->registrationSuccessful() ){
			...
			$inputValues = array();
 	    	$inputValues[ 'referral_id' ] = $this->bean->getPOST('userReferralCode');
