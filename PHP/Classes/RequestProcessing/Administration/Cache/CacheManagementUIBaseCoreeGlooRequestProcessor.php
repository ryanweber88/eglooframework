...
class CacheManagementUIBaseCoreeGlooRequestProcessor extends TemplatePatternRequestProcessor {
	...
	public function populateTemplateVariables() {
		...
		if ( $this->bean->issetGET('action') ) {
			$action = $this->bean->getGET('action');
		} else {
			$action = null;
		}

		if ( $this->bean->issetGET('region') ) {
			$region = $this->bean->getGET('region');
		} else {
			$region = null;
		}

		if ( $this->bean->issetGET('cacheKey') ) {
			$cacheKey = $this->bean->getGET('cacheKey');
		} else {
			$cacheKey = null;
		}
		...
		$this->setTemplateVariable('requestURI', $this->bean->getFullyQualifiedRequestString(array('action','region','cacheKey')));
	...
	public function populateErrorTemplateVariables() {
		...
		$unsetArguments = $this->bean->getUnsetGETArray();
		$errors = array();
