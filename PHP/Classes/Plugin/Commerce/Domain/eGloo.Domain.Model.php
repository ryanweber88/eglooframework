...
abstract class Model extends Delegator
	...
	*/
	...
	public static function __callstatic($name, $arguments) {
		...
		if (preg_match('/^definition_for_field_(.+)$/', $name, $match)) {
			...
			// now lets define out dynamic finder function
			return call_user_func(static::defineMethod( $name, function($__mixed) use ($class, $fields, $findOne, $name) {
				// build string representation of query coinditionals
				$conditions = array();
		...
		if (preg_match('/^constraints$/', $name, $match)) {

			$class   = get_called_class(); //static::classNameFull(); // we don't need generic fake here

			// now lets define out dynamic finder function
			return call_user_func(static::defineMethod( $name, function($__mixed) use ($class, $name) {
				// build string representation of query coinditionals
		...
		if (preg_match('/^constraint_for_field_(.+)$/', $name, $match)) {
		...
		if (preg_match('/^field_definitions$/', $name, $match)) {
