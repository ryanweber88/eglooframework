...
if (empty($_REQUEST)) {
	if (!empty($getArray) && !empty($postArray)) {
		$_REQUEST = array_merge($getArray, $postArray);
	} else if (!empty($getArray)) {
		$_REQUEST = $getArray;
...
function menu_entry($ob,$title) {
	...
	if ($MYREQUEST['OB']!=$ob) {
		return "<li><a href=\"" . eGlooConfiguration::getRewriteBase() . "apc/info?OB=$ob\">$title</a></li>";
...
EOB;
	...
	echo
		graphics_avail() ? 
			  ...
			  "<td class=td-0><img alt=\"\" $size src=\"" . eGlooConfiguration::getRewriteBase() . "apc/info?IMG=1&$time\"></td>".
			  "<td class=td-1><img alt=\"\" $size src=\"" . eGlooConfiguration::getRewriteBase() . "apc/info?IMG=2&$time\"></td></tr>\n"
			: "",
		'<tr>',
...
EOB;
	...
	if (graphics_avail()) {
		...
		echo '
			<img alt="" $size src="' . eGlooConfiguration::getRewriteBase() . 'apc/info?IMG=3&$time" />';
  ...
  echo
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {  
        ...
        echo
          '<tr class=tr-',$i%2,'>',
          "<td class=td-0><a href=\"" . eGlooConfiguration::getRewriteBase() . "apc/info?OB=",$MYREQUEST['OB'],"&SH=",md5($entry[$fieldkey]),"\">",$field_value,'</a></td>',
        ...
        if ($entry['deletion_time']) {
          ...
          echo '[<a href="' . eGlooConfiguration::getRewriteBase() . 'apc/info?' . 'OB=', $MYREQUEST['OB'], '&DU=', urlencode($entry[$fieldkey]), '">Delete Now</a>]';
          echo '</td>';
        } else {
...
EOB;
	...
	if ($list && $i < count($list)) {
		echo "<a href=\"" . eGlooConfiguration::getRewriteBase() . "apc/info?OB=",$MYREQUEST['OB'],"&COUNT=0\"><i>",count($list)-$i,' more available...</i></a>';
	}
